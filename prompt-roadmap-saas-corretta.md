...
| 13-14 | Model routing, multilingua | Model routing: base gettonata (llm_routing cheap/premium con intent; task 13 da completare con budget token legato a Stripe e fallback provider). Fatto a oggi: report giornaliero instradato (task_type=report), retry su fallback anche per report, `data_collection:'deny'` sempre attivo su crea_llm, default non-free (`openai/gpt-4o-mini`). Multilingua non affrontata |
