...
## Corpus Check
- 283 files · ~674,306 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3630 nodes · 5269 edges · 262 communities (222 shown, 40 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 129 edges (avg confidence: 0.64)
...
- render-html.py
- input
- test_reminder_reply.py
- dependencies.py
- Slides Reference
- HTML Slide Template
- HTML Slide Template
- detect_domain
- migrate_airtable_to_bookings.py
...
### Community 3 - "main.py"
Cohesion: 0.08
Nodes (46): middleware, _audit(), conteggio_documenti(), elenca_configurazioni(), elenco_documenti(), elimina_documento_api(), health_check(), _imposta_fonte_dati_per_scheduler() (+38 more)

### Community 4 - "InboundProcessor"
Cohesion: 0.08
Nodes (30): security_audit(), InboundProcessor, contact(), org_id(), asyncio, fixture, test_consent_status_deleted_contact_returns_none(), test_contact_default_consent() (+22 more)
...
Cohesion: 0.06
Nodes (5): main(), CoreRepository, datetime, UUID, Like process_stripe_event but uses an existing connection/transaction so the…
...
### Community 9 - "inbound_processor.py"
Cohesion: 0.10
Nodes (33): costruisci_system_prompt(), costruisci_user_prompt(), formatta_cronologia(), prompts.py ---------- Qui costruiamo il testo che l'agente riceve come…, Trasforma gli ultimi N scambi in testo per il prompt., Genera le istruzioni di ruolo per l'agente, basate sul profilo dell'attività…, Il messaggio del cliente così com'è, con la data odierna per risolvere date…, crea_crew() (+25 more)

### Community 10 - "Repository"
Cohesion: 0.05
Nodes (4): Atomically marks a message as replied+handled. Returns the updated row if this…, Periodic heartbeat — tells the reaper this claim is still alive., Libera i claim rimasti bloccati oltre timeout_minutes. Per i messaggi usa…, Repository
...
Cohesion: 0.13
Nodes (23): main(), AppConfig, load_tenant_config(), UUID, TenantConfig, app_config(), fake_tenant_config(), mock_repo() (+15 more)
...
### Community 46 - "time"
Cohesion: 0.21
Nodes (17): date, test_verifica_disponibilita_esclude_cancellata(), test_verifica_disponibilita_esclude_no_show(), asyncio, test_list_bookings_by_stato(), test_list_bookings_da_verificare(), test_list_bookings_for_reminder(), test_update_booking_payment() (+9 more)
...
Cohesion: 0.20
Nodes (15): run_retention(), avvia_scheduler(), _calendar_sync_job(), genera_e_caching(), _no_show_check_job(), _nonce_cleanup_job(), _reminder_check_job(), _reminder_timeout_job() (+7 more)
...
Cohesion: 0.23
Nodes (13): LLM, crea_review_agent(), crea_review_crew(), crea_review_task(), Agent, Crew, Task, costruisci_system_prompt_review() (+5 more)
...
### Community 88 - "prenotazioni.py"
Cohesion: 0.26
Nodes (15): crea_prenotazione(), ottieni_prenotazioni(), ottieni_semaforo(), ricevi_messaggio(), _coperti_prenotati(), crea_prenotazione_dashboard(), elenco_prenotazioni(), _ora_slot() (+7 more)

### Community 90 - "models/schemas.py"
Cohesion: 0.23
Nodes (12): Enum, chiedi_documenti(), calcola_priorita(), calcola_priorita_recensione(), CanaleMessaggio, ConfiguraEmailInput, DatiPrenotazione, DomandaInput (+4 more)
...
### Community 144 - "post"
Cohesion: 0.20
Nodes (10): carica_documento(), carica_file_documento(), indicizza_documenti(), _prossimo_id(), post, ricevi_recensione(), test_email(), chunk_testo() (+2 more)
...
Cohesion: 0.24
Nodes (7): dedup_check(), Atomic idempotency check via INSERT ON CONFLICT DO NOTHING. Returns True if…, _handle_inbound_message(), _handle_status_update(), Request, Legge il body con limite di dimensione, supporta chunked encoding., _read_limited_body()
...
### Community 149 - "logo/generate.py"
Cohesion: 0.29
...
Cohesion: 0.43
Nodes (6): AsyncClient, get_http_client(), _get_supabase_jwks(), is_demo_mode(), _load_project_env(), verify_supabase_jwt()
...
### Community 176 - "migrate_airtable_to_bookings.py"
Cohesion: 0.52
Nodes (6): fetch_airtable_bookings(), insert_bookings(), main(), _normalize_stato(), _parse_data(), _parse_ora()

### Community 177 - "test_main_integration.py"
Cohesion: 0.48
Nodes (6): skipif, test_cors_fail_closed_on_empty(), test_cors_header_present(), test_cors_whitespace_stripped(), test_health_check(), test_rate_limit_llm_global()

### Community 178 - "test_router.py"
Cohesion: 0.48
Nodes (6): create_router(), app(), app_config(), client(), mock_repo(), fixture
...
## Knowledge Gaps
- **1317 isolated node(s):** `$schema`, `.opencode/plugins/graphify.js`, `fs`, `path`, `fs` (+1312 more)
...
- **Why does `Repository` connect `Repository` to `_headers`, `main.py`, `TestInboxAPI`, `whatsapp/conftest.py`, `CoreRepository`, `scheduler.py`, `apply_status_update`, `AppConfig`, `inbox/routes.py`, `UUID`, `gdpr/routes.py`, `test_repository.py`, `test_gdpr_fk.py`, `test_hitl_repository.py`?**
  _High betweenness centrality (0.022) - this node is a cross-community bridge._
- **Why does `CoreRepository` connect `CoreRepository` to `main.py`, `core/conftest.py`, `scheduler.py`, `time`, `email_service.py`, `gdpr/routes.py`, `webhook_handler.py`, `test_gdpr_fk.py`?**
