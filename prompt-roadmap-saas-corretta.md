...
| 12 | Guardrails e qualità risposta | Fatto — validatore post-LLM (grounding prezzi su RAG/profilo con block+escalation staff, trim lunghezza/markdown), classificatore intent pre-responder (euristica keyword + LLM economico `OPENROUTER_MODEL_INTENT` default cheap, fail-open), cache FAQ semantica pgvector (TTL+invalidazione su upload documento, usage event `cache_hit`), A/B test prompt per tenant deterministico (`GUARDRAIL_AB_VARIANTS`, variant `concise`), feedback loop 👍/👎 (emoji cliente + pulsanti staff nel thread inbox, tabella `message_feedback` migration 031). Corretto anche il bug latente di `messages.handling_type` mai valorizzato (trigger event_log ora calcola `gestito_da_ai` correttamente). Dettagli in `docs/GUARDRAILS.md` |
| 13-14 | Model routing, multilingua | Model routing: base gettonata (llm_routing cheap/premium con intent) + fallback multi-provider no-training (task 13): pass-through provider in `crea_llm` (openrouter/groq/cerebras), deny `data_collection` sempre attivo su OpenRouter, default non-free (`openai/gpt-4o-mini`), report giornaliero instradato con retry su fallback. Id di default allineati ai cataloghi reali (verificato live 2026-08-17): chain `...,groq/openai/gpt-oss-20b`; Cerebras resta supportato in crea_llm ma fuori dalla chain default (account senza credito → 402). Multilingua (task 14, branch task14/multilingua): lingue supportate/default nel profilo onboarding (migration 033, lista chiusa it/en/fr/de/es, "it" sempre attiva), rilevamento lingua delegato al LLM nel system prompt (blocco LINGUE, zero dipendenze), policy su lingue non supportate per verticale: best-effort ovunque, escalation a umano per `studio_medico_dentista`; propagazione lingue alle bozze risposta recensioni (`/api/recensione` + sync Google); passo "Lingue" nel wizard onboarding. Rimandato: budget token legato a Stripe (resta hard ceiling a messaggi) |
| 15 | Infra production-ready | Preparazione deploy-ready completata (task 15, branch task15/infra-deploy-ready) — docker-compose committato: 4 servizi prod (api, worker-inbound, worker-retry, supervisor) con mem+cpu limit e healthcheck profondo `/api/health`; codice morto `reindex_worker.py` e volume `reindex_progress` rimossi; `docs/DEPLOY.md` con procedura VPS+Coolify (env vars da secret manager, requisiti VPS 2 vCPU/4GB, limiti noti: rate limiting in-memory per processo, Redis e multi-replica fuori scope; backup Supabase: scenari Free/Pro documentati + verifica sul dashboard prima del lancio); Dockerfile non-root verificato (`USER appuser`, uid 1000, niente `.env` nell'immagine). Deploy vero rimandato al lancio commerciale (target: VPS+Coolify); Redis e monitoring oltre Sentry rimandati |
| 16 | Debito tecnico | Fatto (task 16, branch task16/debito-tecnico) — verifica 1-1 degli 8 punti noti: Pillow in requirements e usato, README/test/CI/git/DB persistente presenti. Pulizia codice morto: `src/api/routes.py` legacy non montato eliminato; sezione email IMAP obsoleta rimossa dal dashboard (`web/app.js`); backend della feature email rimossa ripulito (`email_config_store.py`, `migrate_email_configs.py`, endpoint deprecati `/api/email/config`, `/api/email/test`, `/api/documenti/indicizza`); `README.md` scritto (architettura, setup, test, deploy, sicurezza). 615 test verdi |
